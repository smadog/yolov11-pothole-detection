---
comments: true
description: Learn how to train YOLOv5 on your own custom datasets with easy-to-follow steps. Detailed guide on yolo_formatted_dataset preparation, model selection, and training process.
keywords: YOLOv5, custom yolo_formatted_dataset, model training, object detection, machine learning, AI, YOLO model, PyTorch, yolo_formatted_dataset preparation, Ultralytics
---
...
```bash
# Example: Train YOLOv5s on the COCO128 yolo_formatted_dataset for 3 epochs
...
4.  **Start Training**: Execute the `train.py` script, providing paths to your `dataset.yaml`, desired pretrained weights (e.g., `yolov5s.pt`), image size, batch size, and the number of epochs.
    ```bash
    python train.py --img 640 --batch 16 --epochs 100 --data path/to/your/yolo_formatted_dataset.yaml --weights yolov5s.pt
    ```
