...
class InfiniteDataLoader(dataloader.DataLoader):
    ...
    for training loops that need to iterate through the yolo_formatted_dataset multiple times without recreating workers.
    ...
    Methods:
        ...
        reset: Reset the iterator, useful when modifying yolo_formatted_dataset settings during training.

    Examples:
        Create an infinite dataloader for training
        >>> yolo_formatted_dataset = YOLODataset(...)
        >>> dataloader = InfiniteDataLoader(yolo_formatted_dataset, batch_size=16, shuffle=True)
    ...
    def reset(self):
        """Reset the iterator to allow modifications to the yolo_formatted_dataset during training."""
...
class _RepeatSampler:
    ...
    over a yolo_formatted_dataset without recreating the sampler.
...
def build_yolo_dataset(
    ...
    """Build and return a YOLO yolo_formatted_dataset based on configuration parameters."""
...
def build_dataloader(dataset, batch: int, workers: int, shuffle: bool = True, rank: int = -1, drop_last: bool = False):
    ...
    Args:
        ...
        shuffle (bool, optional): Whether to shuffle the yolo_formatted_dataset.
    ...
    Examples:
        ...
        >>> yolo_formatted_dataset = YOLODataset(...)
        >>> dataloader = build_dataloader(yolo_formatted_dataset, batch=16, workers=4, shuffle=True)
    """
...
def load_inference_source(source=None, batch: int = 1, vid_stride: int = 1, buffer: bool = False, channels: int = 3):
    ...
    Returns:
        (Dataset): A yolo_formatted_dataset object for the specified input source with attached source_type attribute.

    Examples:
        Load an image source for inference
        >>> yolo_formatted_dataset = load_inference_source("image.jpg", batch=1)

        Load a video stream source
        >>> yolo_formatted_dataset = load_inference_source("rtsp://example.com/stream", vid_stride=2)
    """
    ...
    # Attach source types to the yolo_formatted_dataset
