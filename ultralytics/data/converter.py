...
def convert_coco(
    ...
    """Convert COCO yolo_formatted_dataset annotations to a YOLO annotation format suitable for training YOLO models.

    Args:
        labels_dir (str, optional): Path to directory containing COCO yolo_formatted_dataset annotation files.
        save_dir (str, optional): Path to directory to save results to.
        use_segments (bool, optional): Whether to include segmentation masks in the output.
        use_keypoints (bool, optional): Whether to include keypoint annotations in the output.
        cls91to80 (bool, optional): Whether to map 91 COCO class IDs to the corresponding 80 COCO class IDs.
        lvis (bool, optional): Whether to convert data in lvis yolo_formatted_dataset way.
    ...
    """
    # Create yolo_formatted_dataset directory
...
def convert_segment_masks_to_yolo_seg(masks_dir: str, output_dir: str, classes: int):
    """Convert a yolo_formatted_dataset of segmentation mask images to the YOLO segmentation format.
    ...
    Args:
        ...
        classes (int): Total classes in the yolo_formatted_dataset i.e. for COCO classes=80

    Examples:
        >>> from ultralytics.data.converter import convert_segment_masks_to_yolo_seg

        The classes here is the total classes in the yolo_formatted_dataset, for COCO yolo_formatted_dataset we have 80 classes
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    """Convert DOTA yolo_formatted_dataset annotations to YOLO OBB (Oriented Bounding Box) format.

    The function processes images in the 'train' and 'val' folders of the DOTA yolo_formatted_dataset. For each image,
    it reads the associated label from the original labels directory and writes new labels in YOLO OBB format to a new
    directory.

    Args:
        dota_root_path (str): The root directory path of the DOTA yolo_formatted_dataset.
    ...
    Notes:
        The directory structure assumed for the DOTA yolo_formatted_dataset:
...
def yolo_bbox2segment(im_dir: str | Path, save_dir: str | Path | None = None, sam_model: str = "sam_b.pt", device=None):
    """Convert existing object detection yolo_formatted_dataset (bounding boxes) to segmentation yolo_formatted_dataset
    or oriented bounding box (OBB) in YOLO format. Generate segmentation data using SAM auto-annotator as needed.
    ...
    Notes:
        The input directory structure assumed for yolo_formatted_dataset:
...
def create_synthetic_coco_dataset():
    """Create a synthetic COCO yolo_formatted_dataset with random images based on filenames from label lists.

    This function downloads COCO labels, reads image filenames from label list files, creates synthetic images for
    train2017 and val2017 subsets, and organizes them in the COCO yolo_formatted_dataset structure. It uses
    multithreading to generate images efficiently.
    ...
    LOGGER.info("Synthetic COCO yolo_formatted_dataset created successfully.")
...
def convert_to_multispectral(path: str | Path, n_channels: int = 10, replace: bool = False, zip: bool = False):
    ...
    Examples:
        ...
        Convert a yolo_formatted_dataset
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    """Convert NDJSON yolo_formatted_dataset format to Ultralytics YOLO11 yolo_formatted_dataset structure.
    ...
    Args:
        ndjson_path (Union[str, Path]): Path to the input NDJSON file containing yolo_formatted_dataset information.
        output_path (Optional[Union[str, Path]], optional): Directory where the converted YOLO yolo_formatted_dataset
    ...
    Examples:
        ...
        >>> yaml_path = convert_ndjson_to_yolo("yolo_formatted_dataset.ndjson")
        >>> print(f"Dataset converted to: {yaml_path}")

        Convert with custom output directory:
        >>> yaml_path = convert_ndjson_to_yolo("yolo_formatted_dataset.ndjson", output_path="./converted_datasets")
