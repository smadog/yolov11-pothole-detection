...
# Ultralytics yolo_formatted_dataset *.cache version, >= 1.0.0 for Ultralytics YOLO models
...
class YOLODataset(BaseDataset):
    ...
    Methods:
        cache_labels: Cache yolo_formatted_dataset labels, check images and read shapes.
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        """Cache yolo_formatted_dataset labels, check images and read shapes.
    ...
    def get_labels(self) -> list[dict]:
        ...
        # Check if the yolo_formatted_dataset is all boxes or all segments
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                ...
                "To avoid this please supply either a detect or segment yolo_formatted_dataset, not a detect-segment mixed yolo_formatted_dataset."
            )
            for lb in labels:
                lb["segments"] = []
        if len_cls == 0:
...
class YOLOMultiModalDataset(YOLODataset):
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        if self.augment:
            ...
            # the strategy of selecting negative is restricted in one yolo_formatted_dataset,
    ...
    def category_names(self):
        """Return category names for the yolo_formatted_dataset.
    ...
    def category_freq(self):
        """Return frequency of each category in the yolo_formatted_dataset."""
...
class GroundingDataset(YOLODataset):
    ...
    This yolo_formatted_dataset is designed for grounding tasks where annotations are provided in a JSON file rather
    than the standard YOLO format text files.
    ...
    def verify_labels(self, labels: list[dict[str, Any]]) -> None:
        """Verify the number of instances in the yolo_formatted_dataset matches expected counts.
        ...
        Args:
            labels (list[dict[str, Any]]): List of label dictionaries, where each dictionary contains
                yolo_formatted_dataset annotations. Each label dict must have a 'bboxes' key with a numpy array or
                tensor containing bounding box coordinates.

        Raises:
            AssertionError: If the actual instance count doesn't match the expected count for a recognized
                yolo_formatted_dataset.
    ...
    @property
    def category_names(self):
        """Return unique category names from the yolo_formatted_dataset."""
...
class ClassificationDataset:
    ...
    Attributes:
        ...
        root (str): Root directory of the yolo_formatted_dataset.
        prefix (str): Prefix for logging and cache filenames.

    Methods:
        __getitem__: Return subset of data and targets corresponding to given indices.
        __len__: Return the total number of samples in the yolo_formatted_dataset.
        verify_images: Verify all images in yolo_formatted_dataset.
    """

    def __init__(self, root: str, args, augment: bool = False, prefix: str = ""):
        """Initialize YOLO classification yolo_formatted_dataset with root directory, arguments, augmentations, and
        cache settings.

        Args:
            root (str): Path to the yolo_formatted_dataset directory where images are stored in a class-specific folder
                structure.
            args (Namespace): Configuration containing yolo_formatted_dataset-related settings such as image size,
                augmentation parameters, and cache settings.
            augment (bool, optional): Whether to apply augmentations to the yolo_formatted_dataset.
            prefix (str, optional): Prefix for logging and cache filenames, aiding in yolo_formatted_dataset
                identification.
        """
    ...
    def __len__(self) -> int:
        """Return the total number of samples in the yolo_formatted_dataset."""
        return len(self.samples)

    def verify_images(self) -> list[tuple]:
        """Verify all images in yolo_formatted_dataset.
