...
HELP_URL = "See https://docs.ultralytics.com/datasets for yolo_formatted_dataset formatting guidance."
...
def check_file_speeds(
    ...
    """Check yolo_formatted_dataset file access speed and provide performance feedback.

    This function tests the access speed of yolo_formatted_dataset files by measuring ping (stat call) time and read
    speed. It samples up to 5 files from the provided list and warns if access times exceed the threshold.
    ...
    Examples:
        >>> from pathlib import Path
        >>> image_files = list(Path("yolo_formatted_dataset/images").glob("*.jpg"))
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                assert max_cls < num_cls, (
                    f"Label class {int(max_cls)} exceeds yolo_formatted_dataset class count {num_cls}. "
...
def find_dataset_yaml(path: Path) -> Path:
    """Find and return the YAML file associated with a Detect, Segment or Pose yolo_formatted_dataset.
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    """Download, verify, and/or unzip a yolo_formatted_dataset if not found locally.

    This function checks the availability of a specified yolo_formatted_dataset, and if not found, it has the option to
    download and unzip the yolo_formatted_dataset. It then reads and parses the accompanying YAML data, ensuring key
    requirements are met and also resolves paths related to the yolo_formatted_dataset.

    Args:
        dataset (str): Path to the yolo_formatted_dataset or yolo_formatted_dataset descriptor (like a YAML file).
        autodownload (bool, optional): Whether to automatically download the yolo_formatted_dataset if not found.

    Returns:
        (dict[str, Any]): Parsed yolo_formatted_dataset information and paths.
    """
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            ...
            if s and autodownload:
                ...
                m += f"\nNote yolo_formatted_dataset download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_FILE}'"
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    """Check a classification yolo_formatted_dataset such as Imagenet.

    This function accepts a `yolo_formatted_dataset` name and attempts to retrieve the corresponding
    yolo_formatted_dataset information. If the yolo_formatted_dataset is not found locally, it attempts to download the
    yolo_formatted_dataset from the internet and save it locally.

    Args:
        dataset (str | Path): The name of the yolo_formatted_dataset.
        split (str, optional): The split of the yolo_formatted_dataset. Either 'val', 'test', or ''.

    Returns:
        (dict[str, Any]): A dictionary containing the following keys:

            - 'train' (Path): The directory path containing the training set of the yolo_formatted_dataset.
            - 'val' (Path): The directory path containing the validation set of the yolo_formatted_dataset.
            - 'test' (Path): The directory path containing the test set of the yolo_formatted_dataset.
            - 'nc' (int): The number of classes in the yolo_formatted_dataset.
            - 'names' (dict[int, str]): A dictionary of class names in the yolo_formatted_dataset.
    """
    # Download (optional if yolo_formatted_dataset=https://file.zip is passed directly)
...
class HUBDatasetStats:
    """A class for generating HUB yolo_formatted_dataset JSON and `-hub` yolo_formatted_dataset directory.
    ...
    Args:
        ...
        autodownload (bool): Attempt to download yolo_formatted_dataset if not found locally.

    Attributes:
        task (str): Dataset task type.
        hub_dir (Path): Directory path for HUB yolo_formatted_dataset files.
        im_dir (Path): Directory path for compressed images.
        stats (dict): Statistics dictionary containing yolo_formatted_dataset information.
        data (dict): Dataset configuration data.

    Methods:
        get_json: Return yolo_formatted_dataset JSON for Ultralytics HUB.
        process_images: Compress images for Ultralytics HUB.

    Examples:
        >>> from ultralytics.data.utils import HUBDatasetStats
        >>> stats = HUBDatasetStats("path/to/coco8.zip", task="detect")  # detect yolo_formatted_dataset
        >>> stats = HUBDatasetStats("path/to/coco8-seg.zip", task="segment")  # segment yolo_formatted_dataset
        >>> stats = HUBDatasetStats("path/to/coco8-pose.zip", task="pose")  # pose yolo_formatted_dataset
        >>> stats = HUBDatasetStats("path/to/dota8.zip", task="obb")  # OBB yolo_formatted_dataset
        >>> stats = HUBDatasetStats("path/to/imagenet10.zip", task="classify")  # classification yolo_formatted_dataset
    ...
    def __init__(self, path: str = "coco8.yaml", task: str = "detect", autodownload: bool = False):
        ...
        LOGGER.info(f"Starting HUB yolo_formatted_dataset checks for {path}....")
        ...
        if self.task == "classify":
            ...
            try:
                ...
                data["path"] = ""  # strip path since YAML should be in yolo_formatted_dataset root for all HUB datasets
    ...
    def _hub_ops(self, f: str):
        ...
        compress_one_image(f, self.im_dir / Path(f).name)  # save to yolo_formatted_dataset-hub

    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        """Return yolo_formatted_dataset JSON for Ultralytics HUB."""
        ...
        def _round(labels):
            ...
            if self.task == "detect":
                ...
                raise ValueError(f"Undefined yolo_formatted_dataset task={self.task}.")
        ...
        for split in "train", "val", "test":
            ...
            # Get yolo_formatted_dataset statistics
            if self.task == "classify":
        ...
        if save:
            self.hub_dir.mkdir(parents=True, exist_ok=True)  # makes yolo_formatted_dataset-hub/
    ...
    def process_images(self) -> Path:
        ...
        self.im_dir.mkdir(parents=True, exist_ok=True)  # makes yolo_formatted_dataset-hub/images/
        for split in "train", "val", "test":
...
def compress_one_image(f: str, f_new: str | None = None, max_dim: int = 1920, quality: int = 50):
    ...
    Examples:
        ...
        >>> for f in Path("path/to/yolo_formatted_dataset").rglob("*.jpg"):
...
def save_dataset_cache_file(prefix: str, path: Path, x: dict, version: str):
    """Save an Ultralytics yolo_formatted_dataset *.cache dictionary x to path."""
