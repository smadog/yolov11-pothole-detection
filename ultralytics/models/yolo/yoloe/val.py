...
class YOLOEDetectValidator(DetectionValidator):
    ...
    def get_vpe_dataloader(self, data: dict[str, Any]) -> torch.utils.data.DataLoader:
        ...
        This method prepares a dataloader for visual prompt embeddings (VPE) using the specified yolo_formatted_dataset.
        It applies necessary transformations including LoadVisualPrompt and configurations to the yolo_formatted_dataset
    ...
    def __call__(
        ...
        model. For visual prompts, reference data can be specified to extract embeddings from a different yolo_formatted_dataset.
        ...
        if trainer is not None:
            ...
            if load_vp:
                LOGGER.info("Validate using the visual prompt.")
                self.args.half = False
                # TODO: need to check if the names from refer data is consistent with the evaluated yolo_formatted_dataset
                # could use same yolo_formatted_dataset or refer to extract visual prompt embeddings
