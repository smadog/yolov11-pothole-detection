...
def check_class_names(names: list | dict) -> dict[int, str]:
    ...
    Raises:
        KeyError: If class indices are invalid for the yolo_formatted_dataset size.
    """
    ...
    if isinstance(names, dict):
        ...
        if max(names.keys()) >= n:
            raise KeyError(
                f"{n}-class yolo_formatted_dataset requires class indices 0-{n - 1}, but you have invalid class indices "
                f"{min(names.keys())}-{max(names.keys())} defined in your yolo_formatted_dataset YAML."
            )
